...
public class AbHttpClient {
     ...
     * @param responseListener the response listener
     */
    public void get(final String url, final AbRequestParams params, final AbHttpResponseListener responseListener, final boolean noCookie) {
        ...
        mExecutorService.execute(new Runnable() {
            public void run() {
                try {
                    doGet(url, params, responseListener, noCookie);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
     ...
     */
    public void post(final String url, final AbRequestParams params,
                     final AbHttpResponseListener responseListener, final boolean noCookie) {
        responseListener.setHandler(new ResponderHandler(responseListener));
        responseListener.onStart();
        mExecutorService.execute(new Runnable() {
            public void run() {
                try {
                    doPost(url, params, responseListener, noCookie);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private void doGet(String url, AbRequestParams params, AbHttpResponseListener responseListener) {
        try {
            ...
            HttpGet httpGet = new HttpGet(url);
            httpGet.addHeader(USER_AGENT, userAgent);
            httpGet.addHeader("cookie", F.cookie);
            //压缩
            httpGet.addHeader(ACCEPT_ENCODING, "gzip");
            //取得默认的HttpClient
            HttpClient httpClient = getHttpClient();
     ...
     * @param responseListener the response listener
     */
    private void doGet(String url, AbRequestParams params, AbHttpResponseListener responseListener, boolean noCookie) {
        try {
            ...
            httpGet.addHeader(USER_AGENT, userAgent);
            httpGet.addHeader("cookie", F.cookie);
            httpGet.addHeader("JQDownload", "123");
            //压缩
            httpGet.addHeader(ACCEPT_ENCODING, "gzip");
            //取得默认的HttpClient
            HttpClient httpClient = getHttpClient();
    ...
    private void doPost(String url, AbRequestParams params, AbHttpResponseListener responseListener) {
        try {
            ...
            //压缩
            httpPost.addHeader(ACCEPT_ENCODING, "gzip");
            httpPost.addHeader("cookie", F.cookie);
            //是否包含文件
            boolean isContainFile = false;
            if (params != null) {
                //使用NameValuePair来保存要传递的Post参数设置字符集
     ...
     * @param responseListener the response listener
     */
    private void doPost(String url, AbRequestParams params, AbHttpResponseListener responseListener, boolean noCookie) {
        try {
            ...
            //压缩
            httpPost.addHeader(ACCEPT_ENCODING, "gzip");
            httpPost.addHeader("JQDownload", "123");
            //是否包含文件
            boolean isContainFile = false;
            if (params != null) {
                //使用NameValuePair来保存要传递的Post参数设置字符集
     ...
     * @return
     */
    public void postJson(final String url, final String json, final AbStringHttpResponseListener responseListener) {
        responseListener.setHandler(new ResponderHandler(responseListener));
        responseListener.onStart();
        mExecutorService.execute(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(json)) {
                        ...
                        body = StringBody.create(json, "application/json", Charset.forName("UTF-8"));
                        body.writeTo(urlConn.getOutputStream(), null);
                    } else {
                        urlConn.connect();
                    }
                    if (body != null) {
                        AbLogUtil.i(mContext, "[HTTP POST]:" + url + ",body:" + json);
                    } else {
    ...
    public static class ResponderHandler extends Handler {
         ...
         * 响应消息监听.
         */
        public AbHttpResponseListener responseListener;
